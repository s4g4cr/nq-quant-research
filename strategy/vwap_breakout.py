...
HYPOTHESIS (FALSIFIED):
  VWAP crossover on trending days (OR range >= ATR threshold) combined with
  HMM trending-state filter identifies high-probability momentum breakouts.
  SL = VWAP at entry. TP = trailing stop at 1.5 ATR from peak.

RESULT: Hypothesis rejected.
  Walk-forward validation showed P(success) not significantly above chance.
  Regime routing via HMM improved in-sample but degraded on test windows.
  The structural edge (VWAP as natural S/R) was real but overwhelmed by
  execution costs and the difficulty of timing the entry precisely enough.
  See docs/vwap_research.html for full Phase 9-10 analysis.
