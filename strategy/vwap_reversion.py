"""
VWAP Reversion strategy engine for NQ 1-min (Phase 8).

HYPOTHESIS (FALSIFIED):
  Price extended >= 1.5 ATR from VWAP with an exhaustion candle closing
  toward VWAP represents a high-probability mean-reversion opportunity.
  TP = VWAP at entry. SL = 0.5 ATR from entry.

RESULT: Hypothesis rejected.
  90.7% of VWAP crossovers revert, but signal logic could not separate
  high-probability setups from noise. PF < 1.0 across all parameter sweeps
  on both train and test sets. HMM regime routing (ranging vs trending)
  did not improve results. See docs/vwap_research.html for full analysis.
